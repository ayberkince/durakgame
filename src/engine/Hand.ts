...
export class Hand {
    ...
    has(cardToFind: Card): boolean {
        return this.cards.some((card) => card.isEqual(cardToFind));
    }
    ...
    remove(cardToRemove: Card): boolean {
        const removeIndex = this.cards.findIndex((card) => card.isEqual(cardToRemove));
    ...
    tillFullCount(): number {
        if (this.cards.length >= config.HAND_MAX_CARDS) return 0;
